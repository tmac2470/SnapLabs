...
;(function()
{
	...
	/**
	 * @description Looking for to physical device.
	 * @public
	 */
	...
	/**
	 * @description SensorTag device has been disconnected.
	 * @public
	 */
	status.SENSORTAG_NOT_ONLINE = 'SENSORTAG_NOT_ONLINE'
	...
	sensortag.addInstanceMethods = function(anInstance)
	{
		...
		instance.humidityCallback = function(fun, interval)
		{
			...
			//console.log("DEBUG - Adding humidity service ble-nerld")
			return instance
		}
		...
		{
			//console.log("DEBUG - tisensortag-ble-nerld instance.connectToNearestDevice")
				...
				noTagFoundTimer = setTimeout(
					function() {
						if (!nearestDevice)
						{
							...
							//console.log("DEBUG - SensorTag Not found BBB")
						}
					},
					timeOut)
			}
		 ...
		 * For this to work reliably SensorTags should be at least a
		 * couple of meters apart.
		 * This is valid only for Android where the address of the device is available.
		 * @param scanTimeMilliseconds The time to scan for nearby
		 * SensorTags (optional, defaults to 3 seconds).
		 * @instance
		 * @public
		 */
		instance.connectToSensorTagAddress = function(scanTimeMilliseconds, lookingFor)
		{
			//console.log("DEBUG - tisensortag-ble-nerld instance.connectToSensorTagAddress ")
				...
				noTagFoundTimer = setTimeout(
					function() {
						if (!correctDevice)
						{
							...
							instance.callStatusCallback(
								sensortag.status.SENSORTAG_NOT_FOUND)
							//console.log("DEBUG - SensorTag Not found AAA")
						}
					},
					timeOut)
			}
			...
			function deviceFound(device)
			{
				...
				console.log("Reading Address " +instance.address + "while looking for " + lookingFor);
				
				// Update the device if it has the same SystemID
				if (device.rssi != 127 && instance.deviceIsSensorTag(device) && instance.address == lookingFor)
				{
					console.log('deviceFound: ' + device.name)
		...
		instance.stopScanningForDevices = function()
		{
			...
			//console.log("DEBUG - SensorTag Not found CCC")
		...
		instance.connectToDevice = function(device)
		{
			...
			//console.log("DEBUG - tisensortag-ble-nerld instance.connectToDevice for device " + JSON.stringify(instance.device) )
			//console.log("DEBUG - tisensortag-ble-nerld going to call " + instance.device.connect )
			// This call causes double connection read
			instance.device.connect(
				function(device)
				{
					instance.callStatusCallback(sensortag.status.CONNECTED)
					//console.log("DEBUG - tisensortag-ble-nerld instance.connectToDevice - going to read" )
		...
		instance.disconnectDevice = function()
		{
			if (instance.device)
			{
				instance.callStatusCallback(sensortag.status.SENSORTAG_NOT_ONLINE)
		...
		instance.readDeviceInfo = function()
		{
			function readDeviceInfoService()
			{
				...
				//console.log("DEBUG - readDeviceInfoService avout to call function " + instance.device.readServices)
			...
			function gotDeviceInfoService(device)
			{
				...
				//readModelNumber()
				//TCM Added the reading of the System ID 
				readSystemID()
				readFirmwareVersion()
			}
			...
			*/
			...
			function readRequestedServices()
			{
				...
				//console.log("DEBUG - Required Services are: " + instance.requiredServices.toString());
				// Read services requested by the application.
				instance.device.readServices(
					instance.requiredServices,
			...
			// data successively.
			//console.log("DEBUG - reading device info")
			...
			notificationFunction)
		{
			...
			//console.log("DEBUG - tisensortag-ble-nerld.js values for config are " + configValue + " for sensor " + service.CONFIG)
			configValue && instance.device.writeServiceCharacteristic(
				...
				function() {
					...
					instance.device.readServiceCharacteristic(service.SERVICE, service.CONFIG, function(data) {
						console.log('DEBUG - Service characteristic: ' + instance.dataToString(data) + " for " + service.SERVICE)
					}, function(errorCode)
					{
						console.log('DEBUG - Service characteristic error: ' + errorCode)
					})},
				instance.errorFun)
		...
		{
			//console.log("DEBUG - getting humidity values - ble-nerld")
