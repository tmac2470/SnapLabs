...
;(function()
{
	...
	sensortag.addInstanceMethods = function(anInstance)
	{
		...
		instance.setupCatchAllMovementCallback = function(interval)
		{
			...
			//console.log("DEBUG - tisensortag-ble-cc2650-nerld.js Catching all movement with interval " + interval)
			...
			// Trying to change second byte of configuration to change the range. Set to 8G by default (2), even when writing 2G value (0)
			var value = [127,0]
		...
		instance.movementCallback = function(fun, interval, sensors)
		{
			...
			instance.movementFun = fun
 			...
 			// Set the config that turns on the needed sensors.
			// try set second bit to value for 8G and wake up
			instance.movementConfig = sensors
			instance.movementInterval = interval
			instance.requiredServices.push(instance.MOVEMENT.SERVICE)
 
			//console.log("DEBUG - tisensortag-ble-cc2650-nerld.js values for config are " + instance.movementConfig)
		...
		{
			//console.log("DEBUG - adding Lux service")
		...
		{
			// Changed from node.js version of sensorTag software
			// This depends on rande: 2G = (32768/2), 4G = (32768/4), 8G = (32768/8) = 4096, 16G (32768/16)
			// Written in config values but not sure of what they are for this? 8G (2 always)
			var val = 4096;
			var divisors = {x: -1*val, y: val, z: -1*val}
			//var divisors = {x: 1, y: 1, z: 1}
		...
		instance.getGyroscopeValues = function(data)
		{
			...
			// Changes acc to http://processors.wiki.ti.com/index.php/CC2650_SensorTag_User's_Guide#Movement_Sensor
			var gx = evothings.util.littleEndianToInt16(data, 0) * 500 / 65536.0
			var gy = evothings.util.littleEndianToInt16(data, 2) * 500 / 65536.0
			var gz =  evothings.util.littleEndianToInt16(data, 4) * 500 / 65536.0

			//console.log("DEBUG - Gyroscope values calculated are: " + gx + ", " + gy + ", " +gz)
			// Return result.
		...
		instance.getBarometerValues = function(data)
		{
			...
			// From https://github.com/sandeepmistry/node-sensortag/blob/6a14fea9a1e0a9c8fe7b0343fddfdf6652baab8a/lib/cc2650.js
			...
			if (data.length > 4) {
				...
				flTempBMP = (evothings.util.littleEndianToUint32(data, 0) & 0x00ffffff)/ 100.0;
				flPressure = ((evothings.util.littleEndianToUint32(data, 2) >> 8) & 0x00ffffff) / 100.0;
			} else {
			// Firmware 0.89

				var tempBMP = evothings.util.littleEndianToUint16(data, 0);
				var tempExponent = (tempBMP & 0xF000) >> 12;
				var tempMantissa = (tempBMP & 0x0FFF);
				flTempBMP = tempMantissa * Math.pow(2, tempExponent) / 100.0;

				var tempPressure = evothings.util.littleEndianToUint16(data, 2);
		...
		instance.getMagnetometerValues = function(data)
		{
			...
			// changed acc to http://processors.wiki.ti.com/index.php/CC2650_SensorTag_User's_Guide#Movement_Sensor
